...
Результат — девять файлов в `./output/`:
- `<provider>_service.rb` — сервис по контракту `Provider::BaseService`;
- `INTEGRATION.md` — инструкция по подключению;
- `fixtures.json` — примеры запросов/ответов/уведомлений (контракт v3), проверены
  встроенным валидатором схемы (`generation.json` → `fixtures_schema`);
...
- `generation.json` — машиночитаемый вердикт о допуске (`admitted`, `blockers`) —
  admission-gate для CI/приёмки;
