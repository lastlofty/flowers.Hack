...
require_relative 'paybridge/generators/verdict_generator'
require_relative 'paybridge/generators/ir_dump_generator'
...
module Paybridge
  ...
  def self.generate(spec_path:, provider:, config_path: DEFAULT_CONFIG, overrides_path: nil)
    ...
    files = {
      ...
      'generation.json' =>
        Generators::VerdictGenerator.new(spec, spec_source: source, config: config).render
    }
