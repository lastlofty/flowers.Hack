...
require_relative 'europay_service'

class EuropayServiceTest < Minitest::Test
  ...
  def service(status, body)
    ...
    instance = Provider::EuropayService.new(provider: provider)
  ...
  def assert_auth_header(headers)
    ...
    expected = case auth['type']
               ...
               when 'http'
                 basic = auth['scheme'] == 'basic' ||
                         (!auth.key?('scheme') && auth['credentials_field'] == 'username')
                 if basic
